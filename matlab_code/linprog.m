...
%
%   X = LINPROG(f,A,b,Aeq,beq,LB,UB,OPTIONS) minimizes with the
...
%   PROBLEM.options, and solver name 'linprog' in PROBLEM.solver.
%
...
% x0 is no longer supported.
if nargin == 9
    oldSyntax = "LINPROG(f,A,b,Aeq,beq,LB,UB,X0,OPTIONS)";
    % newSyntax = "LINPROG(f,A,b,Aeq,beq,LB,UB,OPTIONS)";
    error(message('optim:linprog:X0NotAccepted', oldSyntax, newSyntax));
elseif nargin == 8
    if isa(x0, 'struct') || isa(x0, 'optim.options.SolverOptions')
        options = x0;
    else
        oldSyntax = "LINPROG(f,A,b,Aeq,beq,LB,UB,X0)";
        newSyntax = "LINPROG(f,A,b,Aeq,beq,LB,UB)";
        error(message('optim:linprog:X0NotAccepted', oldSyntax, newSyntax));
    end
end
...
if nargin == 1
    if isa(f,'struct')
        problemInput = true;
        [f,A,B,Aeq,Beq,lb,ub,~,options] = separateOptimStruct(f);
...
algDSX   = 'dual-simplex-legacy';
algDSXHi = 'dual-simplex-highs';
algIP15b = 'interior-point';

% Check for non-double inputs
msg = isoptimargdbl(upper(thisFcn), {'f','A','b','Aeq','beq','LB','UB'}, ...
                                      f,  A,  B,  Aeq,  Beq,  lb,  ub);
if ~isempty(msg)
...
if ~algIsEmpty
    ...
    if ~any(strcmp({algIP; algDSX; algDSXHi; algIP15b},Algorithm))
...
[~,lb,ub,msg] = checkbounds([],lb,ub,nvars);
if ~isempty(msg)
   exitflag = -2;
   x = []; fval = []; lambda = [];
   output.iterations = 0;
...
options = optim.internal.utils.ObjectiveSenseManager.setupLinprogManager(options,Algorithm);
...
elseif strcmpi(Algorithm,algDSX) || strcmpi(Algorithm,algDSXHi) || strcmpi(Algorithm,algIP15b)
    ...
    % Set ProblemdefOptions
    if isfield(options, 'ProblemdefOptions')
        algoptions = setProblemdefOptions(algoptions, options.ProblemdefOptions);
    end
    ...
    if strcmpi(Algorithm,algDSXHi)
        algoptions.Presolve = optimget(options,'Presolve',defaultopt,optimgetFlag);
        algoptions.MaxTime = optimget(options,'MaxTime',defaultopt,optimgetFlag);
    end
