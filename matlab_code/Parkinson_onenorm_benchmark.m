...
filename = 'parkinsons.csv.csv'; % Replace with your Parkinson's dataset file name
if ~isfile(filename)
...
X = data{:, [1:4, 6:end]};  % Predictor variables (excluding the 5th column which is the target)
Y = data{:, 5};             % Response variable (target in the 5th column)

% Parameters for epsilon quantile SVR
s = 2^50;              % Kernel parameter
c1 = 2^-2.96;           % Regularization parameter
...
% Save to CSV file
writetable(cell2table(results1), 'results_PI_onenorm_quantileLPONENORM_parkinsons.csv');
