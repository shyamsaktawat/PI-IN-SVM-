...
filename = 'Concrete.xlsx'; % Replace with your CSV file name
...
s = 2^7        ;              % Kernel parameter -7 TO 7
c1 = 2^-8 ;             % Regularization parameter -10 TO 10
eps1 = 1;               % Epsilon value for SVR
lower_quantiles = [0.010, 0.015, 0.020, 0.025, 0.030];

% Train-test split (70% training, 30% testing)
N = size(X, 1);
n = round(0.2 * N);  % Number of test points
trainX = X(1:N-n,:);
ytrain = Y(1:N-n,:);
testX = X(N-n+1:end,:);
ytest = Y(N-n+1:end,:);
...
% Save to CSV file
writetable(cell2table(results1), 'result_PI_Concrete_epsilon.csv');
