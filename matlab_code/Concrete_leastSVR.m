...
filename = 'Concrete.xlsx';  % Replace with your actual file name
data = readtable(filename);  % Load the data

% Step 2: Extract features (X) and response (Y)
X = table2array(data(:, 1:end-1));  % All columns except the last one (features)
...
kerfPara.pars = 2^10.5;      % Kernel parameter (gamma for RBF)
C = 2^8.5;               % SVR with tau = 0.5 for standard regression
